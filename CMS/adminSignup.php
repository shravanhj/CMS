...
if(isset($_POST['submit'])){
    ...
    $check_user = $conn->prepare("SELECT * FROM `admins` WHERE Email = ? OR Phone_No = ?");
    $check_user->execute([$_SESSION['email'], $_SESSION['phonenumber']]);
    $select_user = $check_user->fetch(PDO::FETCH_ASSOC);

    if($check_user->rowCount() > 0){
        echo "<script type='text/javascript'>alert('Account Already Registered.');window.location.href='adminSignup.php';</script>";  
    }
    else{
        $insert_admin = $conn->prepare("INSERT INTO `admins` (Name, Email, Phone_No, Password) VALUE (?, ?, ?, ?)");
        $insert_admin->execute([$_SESSION['fullname'], $_SESSION['email'], $_SESSION['phonenumber'], $_SESSION['password']]);
        echo "<script type='text/javascript'>alert('Account Created Successfully.');window.location.href='adminSignup.php';</script>";
    }
}
?>
...
<form method="post">
    <h4 style="padding:5px; text-align:left; border-bottom:2px solid black;">CSM Admin Registration</h4>
    <div class="mb-1 mt-3">
...
<div class="label fs-6">
    <label for="logIn" class="label">Already Registered? <a href="adminLogin.php" style="text-decoration:none;">Login</a></label>
</div>
</div>
</div>
<script>
